...
namespace Open.Utils.Excel
{
    ...
    public class ExcelHelper
    {
        ...
        public ActionResult ExcelDownload<T>(List<T> list, string fileName)
        {
            var columnList = TojqGridModel<T>();
            var excelConfig = ConvertExcelGridModelToConfig(columnList, fileName);
            var rowData = ToDataTable(list);
        ...
        public ActionResult ExcelDownload(DataTable dtSource, List<jqGridModel> columnList, string fileName)
        {
            var excelConfig = ConvertExcelGridModelToConfig(columnList, fileName);
            var stream = ExportMemoryStream(dtSource, excelConfig);
            return new FileStreamResult(stream, MIMEType.xls) { FileDownloadName = JointXls(excelConfig.FileName) };
        }
        ...
        /// <param name="dtSource">DataTable数据源</param>
        /// <param name="excelConfig">导出设置包含文件名、标题、列设置</param>
        public ActionResult ExcelDownload(DataTable dtSource, ExcelConfig excelConfig)
        {
            var stream = ExportMemoryStream(dtSource, excelConfig);
            return new FileStreamResult(stream, MIMEType.xls) { FileDownloadName = JointXls(excelConfig.FileName) };
        }
        ...
        private DataTable ReadSheetToDataTable(int headerRowNo, ISheet sheet)
        {
            ...
            try
            {
                for (i = (headerRowNo + 1); i <= sheet.LastRowNum; i++)
                {
        ...
        private ExcelConfig ConvertExcelGridModelToConfig(List<jqGridModel> columnList, string fileName)
        {
            ...
            foreach (jqGridModel columnModel in columnList)
            {
        ...
        #region 获取实体对象备注，赋值到列名
        /// <summary>
        /// 获取实体对象备注，赋值到列名
        /// </summary>
        /// <typeparam name="T">实体</typeparam>
        /// <returns>数据集(表)</returns>
        public static List<jqGridModel> TojqGridModel<T>()
        {
            PropertyInfo[] properties = typeof(T).GetProperties(BindingFlags.Public | BindingFlags.Instance);
            List<jqGridModel> _jqGridModel = new List<jqGridModel>();
            foreach (PropertyInfo property in properties)
            {
                object[] proDescrition = property.GetCustomAttributes(typeof(DescriptionAttribute), true);
                if (proDescrition.Length > 0)
                {
                    _jqGridModel.Add(new jqGridModel()
                    {
                        name = property.Name,
                        label = ((DescriptionAttribute)proDescrition[0]).Description
                    });
                }
                else
                {
