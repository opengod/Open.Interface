...
namespace Open.Utils
{
    public class RedisCacheManager : IRedisCacheManager
    {
        ...
        #region 构造函数
        public RedisCacheManager(int dbNum = 0) : this(dbNum, null)
        {

        }

        public RedisCacheManager(int dbNum, string readWriteHosts)
        {
            DbNum = dbNum;
            // 是否启用Redis
            var enabled = Appsettings.app(new string[] { "AppSettings", "RedisCaching", "Enabled" });
            if (bool.Parse(enabled))
            {
                _conn = string.IsNullOrWhiteSpace(readWriteHosts) ? RedisConnection.Instance : RedisConnection.GetConnectionMultiplexer(readWriteHosts);
            }
        }
        #endregion 构造函数
        ...
        {
            return _conn.GetDatabase().KeyExists(key);
        }
        ...
        {
            var value = _conn.GetDatabase().StringGet(key);
            if (value.HasValue)
            {
                return SerializeHelper.ConvertObj<T>(value);
            }
            else
            {
                return default(T);
        ...
        public void Set(string key, object value, TimeSpan cacheTime)
        {
            if (value != null)
            {
                _conn.GetDatabase().StringSet(key, SerializeHelper.Serialize(value), cacheTime);
            }
        }
        ...
        {
            return _conn.GetDatabase().StringSet(key, value, TimeSpan.FromSeconds(120));
        }
